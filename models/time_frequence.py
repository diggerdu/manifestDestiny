...
class ifft(nn.Module):
    def __init__(self, nfft=1024):
        ...
        self.real_conv = nn.Conv2d(
            1, nfft, (n_freq, 1), stride=1, padding=0, bias=False)
...
class istft(nn.Module):
    def __init__(self, nfft=1024, hop_length=512):
        ...
        self.trans_kernels = nn.Parameter(
            torch.from_numpy(trans_kernels[:, np.newaxis, np.newaxis, :])
            .float())
    ...
    def forward(self, magn, phase, ac):
        ...
        output = F.conv_transpose2d(
            output, self.trans_kernels, stride=self.hop_length)
...
if __name__ == '__main__':
    ...
    signal = np.ones((4096, ))
    input_ = Variable(
        torch.from_numpy(signal[np.newaxis, np.newaxis, np.newaxis, :])
        .float())
